...
class Tree
{
    ...
    public function getTreeMenu($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $value = array(
                    '@childlist' => $childlist,
                    '@href'       => $childdata || !isset($value['@url']) ? "javascript:;" : url($value['@url']),
    ...
    //制作一级菜单
    public function getTreeMenu1($myid)
    {
        $str = '';
        $childs = $this->getChild($myid);
        if ($childs) {
            foreach ($childs as $value) {
                $id = $value['id'];
                $ch = $this->getChild($value['id']);
                if (count($ch)>0) {
                    $li = '<li data-name="@id" class="layui-nav-item"><a href="javascript:;" lay-tips="@title" lay-direction="2">
                            <i class="layui-icon @icon"></i>
                            <cite>@title</cite>
                        </a><dl class="layui-nav-child">@chield</dd></dl><li>';
                } else {
                    $li = '<li data-name="@id" class="layui-nav-item"><a href="javascript:;" lay-href="/@href" lay-tips="@title" lay-direction="2">
                            <i class="layui-icon @icon"></i>
                            <cite>@title</cite>
                        </a><li>';
                }
                ...
                $char = $this->getTreeMenu(
                    $id,
                    '<dd class="@title" ><a lay-href="/@href">@title</a> @childlist</dd>',
                    1,
                    '',
                    'dl',
    ...
    public function getTreeNode($myid,$check=[])
    {
        $childs = $this->getChild($myid);
        $n = 0;
        $data = [];
        ...
        if ($childs) {
            foreach ($childs as $id => $value) {
                ...
                $data[$n] = $value;
                $data[$n]['children'] = $this->getTreeNode($id,$check);
                $n++;
                $number++;
            }
        }
    ...
    public function getTopNode($arr,$check)
    {

        foreach ($arr as $id => &$value) {
            $ids = array_map('array_shift', $check);
            $a = array_intersect($ids,$check);
            $childs = $this->getChild($value['id']);
            if(count($childs)!=count($a)){
                $value['checked'] = false;
            }
            $this->getTopNode($value,$check);
        }
        return $arr;
    }
    /**
