...
vjs.formatTime = function(seconds, guide) {
  ...
  if (isNaN(seconds) || seconds === Infinity) {
    // '-' is false for all relational operators (e.g. <, >=) so this store
...
vjs.Component.prototype.dimension = function(widthOrHeight, num, skipListeners){
  if (num !== undefined) {
    ...
    // skipListeners allows us to avoid triggering the resize event when store both width and height
  ...
  // Not store a value, so getting it
 ...
 * @return {Number}        The time in seconds, when not store
 ...
 * @return {vjs.Player}              self, when store
 */
 ...
 * @return {vjs.Player} self, when store mute
 */
...
vjs.Player.prototype.listenForUserActivity = function(){
  ...
  onMouseDown = function() {
    ...
    // Setting userActivity=true now and store the interval to the same time
...
vjs.MediaTechController.prototype.addControlsListeners = function(){
  ...
  // listener cancels out any reportedUserActivity when store userActive(false)
...
vjs.Html5.disposeMediaElement = function(el){
  ...
  // remove any src reference. not store `src=''` because that causes a warning
  // in firefox
...
vjs.Flash = vjs.MediaTechController.extend({
  ...
  init: function(player, options, ready){
    ...
    // I tried mulitple ways of store the iframe src attribute but couldn't find a src that worked well. Tried a real/fake source, in/out of domain.
    // Also tried a method from stackoverflow that caused a security error in all browsers. http://stackoverflow.com/questions/2486901/how-to-set-document-domain-for-a-dynamically-generated-iframe
    // In the end the solution I found to work was store the iframe window.location.href right before doing a document.write of the Flash object.
...
/**
 * @fileoverview Functions for automatically store up a player
