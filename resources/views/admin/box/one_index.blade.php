...
@section('page-content')
    <div class="layui-form">
        <button class="layui-btn layui-btn-normal layui-btn-radius" id="add_type_one">添加普通赏</button>
    ...
    <script type="text/javascript">
        window.onload = function () {
            layui.use(['layer', 'table','form','laydate'], function () { //独立版的layer无需执行这一句
                ...
                function tbRend(url) {
                    table.render({
                        ...
                        cols: [[
                            ...
                            {field: 'num', title: '箱数', minWidth: 100},
                ...
                tbRend("{{url('/admin/box_one_list')}}");
                $("#search").click(function() {
                    var name = $("input[ name='name']").val();
                    tbRend("{{url('/admin/box_one_list')}}?name="+ name);
                    return false;
                });
                form.on('switch(state)', function (obj) {
                    var id = obj.value;
                    $.ajax({
                        url: '{{url('admin/box_one_state')}}',
                        type: 'post',
                        dataType: 'json',
                        data: {id: id},
                ...
                table.on('tool(phoneList)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'edit'){ //编辑
                        ...
                        layer.open({
                            ...
                            maxmin: true,
                            title:'编辑普通赏',
                            content: '/admin/box_one_add?id=' + data.id
                        });
                    }else if(layEvent === 'del'){ //删除
                        layer.confirm('真的要删除吗？', function(index){
                            //向服务端发送删除指令
                            $.ajax({
                                url:'/admin/box_one_del',
                                type:'post',
                                dataType:'json',
                                data:{id:data.id},
                                success:function(res){
