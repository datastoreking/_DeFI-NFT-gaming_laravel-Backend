...
@section('page-content')
    <div class="layui-form">
        <button class="layui-btn layui-btn-normal layui-btn-radius" id="add_ad">新增赏品级别</button>
    </div>
    ...
    <script type="text/javascript">
        window.onload = function () {
            layui.use(['layer', 'table','form','laydate'], function () { //独立版的layer无需执行这一句
                ...
                $('#add_ad').click(function(){layer_show('新增赏品级别', '/admin/level_add',800,600);});
                function tbRend(url) {
                    table.render({
                        elem: '#adList',
                        url: url,
                ...
                table.on('tool(adList)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'edit'){ //编辑
                        //do something
                        layer_show('修改轮播图', '/admin/level_add?id=' + data.id,800,600);
