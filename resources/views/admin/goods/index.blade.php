...
@section('page-content')
    <div class="layui-form">
        <button class="layui-btn layui-btn-normal layui-btn-radius" id="add_phone">添加赏品</button>
    ...
    <script type="text/javascript">
        window.onload = function () {
            layui.use(['layer', 'table','form','laydate'], function () { //独立版的layer无需执行这一句
                ...
                function tbRend(url) {
                    table.render({
                        ...
                        cols: [[
                            ...
                            {field: 'cost_price', title: '回收价', minWidth: 100},
                            {field: 'sort', title: '排序值', width: 100},
                ...
                tbRend("{{url('/admin/goods_list')}}");
                $("#search").click(function() {
                    var id = $("input[ name='id']").val();
                    var name = $("input[ name='name']").val();
                    tbRend("{{url('/admin/goods_list')}}?id="+ id + "&name=" + name);
                    return false;
                });
                //监听工具条
                table.on('tool(phoneList)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'edit'){ //编辑
                        ...
                        layer.open({
                            ...
                            maxmin: true,
                            title:'编辑赏品',
                            content: '/admin/goods_add?id=' + data.id
                        });
