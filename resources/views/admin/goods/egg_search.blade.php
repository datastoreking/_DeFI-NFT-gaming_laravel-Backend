    ...
    <script type="text/javascript">
        window.onload = function () {
            layui.use(['layer', 'table','form','laydate'], function () { //独立版的layer无需执行这一句
                ...
                tbRend("{{url('/admin/goods_egg_search')}}");
                $("#search").click(function() {
                    var id = $("input[ name='id']").val();
                    var name = $("input[ name='name']").val();
                    tbRend("{{url('/admin/goods_egg_search')}}?goods_id="+ id + "&goods_name=" + name);
                    return false;
                });
                table.on('tool(phoneList)', function(obj){
                    ...
                    if(obj.event === 'add') {
                        ...
                        _tr += "<tr>" +
                            ...
                            "<td><input type='text' style='width: 120px;' class='ratio' name='number[]'></td>"+
                            "<td><input type='text' style='width: 120px;' class='level' name='is_special[]'></td>"+
